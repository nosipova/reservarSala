package es.sdos.oficina.reservasalas.util;
...
@Component
@Slf4j
public class ReservaSalaUtils implements ReservarSalasConstant {
	...
	public <F, T> List<T> mapList(List<F> fromList, final Class<T> toClass) {
		return fromList.stream().map(from -> this.dozerBeanMapper.map(from, toClass)).collect(Collectors.toList());
	}
